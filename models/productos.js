...
class Producto {
    ...
    async save( element ){
        try {
            let _respuesta = await fs.promises.readFile(`../utils/${this.archivo}.txt`,'utf-8');
            ...
            try {
                await fs.promises.writeFile(`../utils/${this.archivo}.txt`, JSON.stringify(_listadoProductos,null,2));
            ...
            try {
                await fs.promises.writeFile(`../utils/${this.archivo}.txt`,JSON.stringify([producto],null,2));
                return producto.id;
            } catch (err) {
                return {
                    status: 'Error', 
    ...
    async deleteById(id) {
        try{
            ...
            try {
                await fs.promises.writeFile(`../utils/${this.archivo}.txt`,JSON.stringify(productosActualizados,null,2))
                return {
                    status: 'Success', 
    ...
    async deleteAll() {
        try{
            // await fs.promises.unlink(`../utils/${this.archivo}.txt`);

            let archivosBorrados = [];
            await fs.promises.writeFile(`../utils/${this.archivo}.txt`,JSON.stringify(archivosBorrados,null,2))
...
const _verListado = new Producto('productos');
